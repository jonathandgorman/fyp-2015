...
% for k = 1:20 % loop for repeated output file generation
    ...
    disp('Simulation started - ULA With an Inactive Element Beam Pattern Synthesis Using a Metropolis Algorithm')
    disp(date)
    ...
    % generate initial neampattern for array with broken elements
    ...
    maxIters = 700000; % maximum number of iterations
    maxTime = 10; % max time in seconds for timed tests
    minBR = 0.180; % minimum value of BR for performance testing
    ...
    for n = 1:maxIters
        ...
        [ML, maxSL,BR] =  peakFinderULAInactiveElement(erMetropolis); % measure corresponding beampattern characteristics
        ...
        prob = exp((-(gaussBR - optBR)/Tarray(n))); % calculate out probability of acceptance
    ...
    %% End of simulation
    str1 = sprintf('./Metros_ULA_Inactive_Elements_matfiles/BR_%d_Metropolis_ULA_Inactive_Element_ters_%d_time_%d',optBR,iters,endTime);
