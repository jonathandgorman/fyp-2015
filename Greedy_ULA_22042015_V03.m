...
% for k = 1:20 % loop for repeated output file generation
    ...
    disp('Simulation started - ULA Beam Pattern Synthesis Using a Greedy Algorithm')
    disp(date)
    ...
    R = 20; % relative distance between sidelobe level and mainbeam in dB, (e.g., R = 20)
    ...
    % creates a phased array pattern
    [erPhased] = phasedArray(d,phi,N,nAngle);
    ...
    for n = 1:maxIters
        ...
        [ML, maxSL, BR] = peakFinderULA(erGreedy); % array parameters calculated for new array pattern
        ...
        if ((toc >= maxTime)||(optBR <= minBR)) % if current time is greater than time limit OR minimum target BR reached
    ...
    erGreedy = er*exciteOptimum'; % set optimised excitation current vecto
    ...
    xlabel('Angle \theta (degrees)','FontSize', 25)
    ylabel('Gain (dB)','FontSize', 25)
    hLegend = legend('Initial ULA Beam Pattern','Greedy Optimised Beampattern');
    set(hLegend,'FontSize',15);
    hold off
    ...
    % decibel plot comparing the initial beampattern with the optimised beampattern
    figure
    plot(rad2deg(thetaArray),(20*log10(abs(abs(er*abs(aArray')))/max(abs(er*abs(aArray'))))),'k')
    grid on
    ylim([-40,5])
    xlabel('Angle \theta (degrees)','FontSize', 25)
    ylabel('Gain (dB)','FontSize', 25)
    hLegend = legend('Dolph-Chebyshev Beam Pattern');
    set(hLegend,'FontSize',15);
    hold off
    ...
    %% End of simulation
    str1 = sprintf('./Greedy_ULA_matfiles/boundedPerformance_%d_Greedy_ULA_iters_%d_time_%d',optBR,iters,endTime);
